...
function submenu_v($member_id,$dbObj)
{
	...
	if(mysqli_num_rows($rs_children)>0)
	{
        ...
        <li style="padding-top: 4px;">               	
          <a><?php echo $rowChild[cl_first_name]; echo (($rowChild[cl_spouse_name]!=NULL)? " & ".$rowChild[cl_spouse_name]:"");?></a>
          <?php
          if(submenu_v($rowChild[cl_member_id],$dbObj)=="")
          {
              ?></li><?php
          }
		}
...
<div style="border:1px solid #09F;border-radius:10px;font-family: Arial, Helvetica, sans-serif;font-size: 13px;">
<!-- <div class="tree"> -->
<!-- <table style="border:1px solid #09F;border-radius:10px;font-family: Arial, Helvetica, sans-serif;font-size: 13px;"> -->
<!-- <tr><td> -->
<ul>
<?php
while($row_topmost = mysqli_fetch_array($rs_topmost))
{
	?>
    	<li>
        	<a><?php echo $row_topmost[cl_first_name];echo (($row_topmost[cl_spouse_name]!=NULL)? " & ".$row_topmost[cl_spouse_name]:"");?></a>
		<?php
        if(submenu_v($row_topmost[cl_member_id],$dbObj)=="")
        {
            ?></li><?php
        }
}
